module.exports = [
"[project]/.next-internal/server/app/api/materials/orders/route/actions.js [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/src/app/api/materials/orders/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        let query = supabaseService.from('material_orders').select(`
        id,
        project_id,
        material_id,
        quantity,
...
async function POST(request) {
    try {
        ...
        const { project_id, warehouse_location, supplier_material_id, material_id = supplier_material_id, quantity, unit_price_eur, unit_price = unit_price_eur, delivery_date, expected_delivery_date = delivery_date, supplier, status = 'pending', notes } = body;
        // Validation
        ...
        const { data: supplierMaterial } = await supabaseService.from('supplier_materials').select('material_id').eq('id', material_id).single();
        if (supplierMaterial) {
            ...
            const { data: directMaterial } = await supabaseService.from('materials').select('id').eq('id', material_id).single();
            if (!directMaterial) {
                return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        const { data: order, error: orderError } = await supabaseService.from('material_orders').insert([
            orderData
        ]).select().single();
        if (orderError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e8ed15c8._.js.map