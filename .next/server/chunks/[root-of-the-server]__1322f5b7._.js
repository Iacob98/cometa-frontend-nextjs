module.exports = [
"[project]/.next-internal/server/app/api/geospatial/features/route/actions.js [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__, module, exports) => {
...
"[project]/src/app/api/geospatial/features/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    try {
        ...
        let query = supabase.from("geospatial_features").select(`
        id,
        type,
        geometry,
        properties,
      ...
      `);
        ...
        const { count } = await supabase.from("geospatial_features").select("*", {
            count: "exact",
            head: true
        });
        const { data: features, error } = await query.range((page - 1) * per_page, page * per_page - 1).order("created_at", {
            ascending: false
        });
        if (error) {
...
async function POST(request) {
    try {
        ...
        const { data: feature, error } = await supabase.from("geospatial_features").insert([
            featureData
        ]).select().single();
        if (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__1322f5b7._.js.map